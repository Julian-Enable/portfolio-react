...
const CardSwap = ({
  ...
  const refs = useMemo(
    () => Array.from({ length: childArr.length }, () => ({ current: null })),
    [childArr.length]
  );
  ...
  useEffect(() => {
    ...
    const swap = () => {
      ...
      // Llamar al callback INMEDIATAMENTE antes de la animación
      if (onCardChange) {
        onCardChange(rest[0]); // El nuevo índice frontal
      }
...
CardSwap.propTypes = {
  ...
  easing: PropTypes.oneOf(['elastic', 'power']),
